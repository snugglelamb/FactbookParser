...
class Factbook
  ...
  # hash_country ={}
  def parse_country(hash_country)
  ...
  # hash_country = parse_country(hash_country)
  ...
  def compare_xml(hash_xml,hash_region)
    ...
    p "#{flagf} mismatches found, other #{flagt} regions are correct"
    # print "\n"
  end
  ...
  def parse_elevation(hash_country,eur,hash_elevation)
    ...
    hash_elevation.map {|country| p "#{country[0]}: #{country[1]}"}
    # print "\n"
  end
  ...
  def parse_elec(hash_elec,hash_popu, hash_avrelec,top)  
    store =[]
    puts "Find top #{top} countries with highest electricity consumption per capita"
      ...
      if item[1] =~ /dominant/
        ...
        if item[1].match(/\d+.?(\d+)?%/)[0].to_f >= above
          if item[1] =~ /,/
            ...
            store << [item[0], item[1].match(/\d+.?(\d+)?%/)[0].to_f]
          else
            puts " #{item[0]};  #{item[1]}"
            store << [item[0], item[1].match(/\d+.?(\d+)?%/)[0].to_f]
          end
  ...
  def parse_lock(hash_lock, hash_bound)
    ...
    hash_lock.each do |item|
      ...
      if item[1].length == 1
        ...
        store << "#{item[0]} Entirely landlocked by: #{item[1][0].strip}"
      end
    end
    # print "\n"
  end
  ...
  def parse_internet(hash_internet,hash_popu, hash_internetpercent)  
    ...
    hash_internetpercent[0..4].map {|country| p "#{country[0]}: #{(country[1] * 100).round(2)}%" }
    # print "\n"
  end
  ...
  def draw_scatterplot(hash_internetpercent, hash_avrelec)
    ...
    plot.export_html
    p "plot stored in local directory next to your ruby file."
...
# added execution part, don't need to be tested
# since it uses methods defined in the class which has already been tested
# just to make sure the program still works after wrapping it with class Factbook

# def start
#   factbook = Factbook.new
#   #parse country url
#   hash_country ={}
#   factbook.parse_country(hash_country)
#
#   #parse region, coordinates, population, internet coverage
#   hash_region = {}
#   hash_coor = {}
#   hash_popu = {}
#   hash_internet = {}
...
#
#   #parse region info through sourceXML
#   hash_xml ={}
#   factbook.parse_xml(hash_xml)
#   factbook.compare_xml(hash_xml,hash_region)
#
#   # earth quake
#   target_region = "South America"
#   soa = hash_region.map{|key| key[0] if key[1] == target_region}.compact.uniq
#   factbook.parse_earthquake(hash_country,soa)
#
#   # Europe elevation
#   target_region = "Europe"
#   eur = hash_region.map{|key| key[0] if key[1] == target_region}.compact.uniq
#   hash_elevation = {}
#   factbook.parse_elevation(hash_country,eur,hash_elevation)
#
#   # list countries in southeastern hemisphere
#   # S, E for southeastern, N E for northeastern etc.
#   hemisphere = "southeastern"
#   factbook.parse_hemisphere(hash_coor, hemisphere)
#
#   # List countries in Asia with >10 political parties
#   # find keyword Asia, can be changed to Europe etc.
#   asia = hash_region.map{|key| key[0] if key[1] =~ /Asia/}.compact.uniq
...
#   # set threshold
#   threshold = 10
#   factbook.parse_political(hash_political,asia,hash_country, threshold)
#
#   # Find top N countries with highest electricity consumption per capita
#   # get electricity consumption
#   hash_elec = {}
#   hash_avrelec = {}
#   top = 5
#   factbook.parse_elec(hash_elec,hash_popu, hash_avrelec,top)
#
#   # list countries with religion, which covers >80% population or <50% population
#   hash_religion = {}
#   above = 80
#   below = 50
#   factbook.parse_religion(hash_religion, above, below)
#
#   # landlock
#   hash_lock ={}
#   hash_bound = {}
#   factbook.parse_lock(hash_lock, hash_bound)
#
#   #internet coverage
#   hash_internetpercent = {}
#   factbook.parse_internet(hash_internet,hash_popu, hash_internetpercent)
#
#   #draw scatterplot
#   factbook.draw_scatterplot(hash_internetpercent, hash_avrelec)
# end
#
# start
